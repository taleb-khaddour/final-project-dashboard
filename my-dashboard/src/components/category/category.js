...
import EditCategory from './categoryEdit/index.js'
...
export default function Menu() {
              ...
              {iconEdit && (
                <Button
                  ...
                  onClick={() => {
                    axios
                      .get(
                        `https://idecream-backend.onrender.com/api/category/${tableMeta.rowData[0]}`,
                      )
                      .then((response) => {
  ...
  const getData = () => {
    axios
      .get(`https://idecream-backend.onrender.com/api/category`)
      .then((response) => {
  ...
  const EditData = () => {
    axios
      .patch(
        `https://idecream-backend.onrender.com/api/Category/${Id}`,
        DataEdit,
      )
      .then((res) => {
        console.log(res)
  ...
  return (
    <div className="incomss">
      {visibleEdit && (
        <EditCategory allData={DataById} getData={getData} showEdit={showEdit} />
      )}
      ...
      {formPopUp && <AddCategory ShowPopUp={ShowPopUp} getData={getData} />}
    </div>
  )
}
